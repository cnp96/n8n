...
export class License implements LicenseProvider {
	...
	getValue<T extends keyof FeatureReturnType>(feature: T): FeatureReturnType[T] {
		if (feature === LICENSE_QUOTAS.USERS_LIMIT)
			return UNLIMITED_LICENSE_QUOTA as FeatureReturnType[T];
		if (feature === LICENSE_QUOTAS.TRIGGER_LIMIT)
			return UNLIMITED_LICENSE_QUOTA as FeatureReturnType[T];
		if (feature === LICENSE_QUOTAS.VARIABLES_LIMIT)
			return UNLIMITED_LICENSE_QUOTA as FeatureReturnType[T];
		if (feature === LICENSE_QUOTAS.AI_CREDITS) return 999999 as FeatureReturnType[T];
		if (feature === LICENSE_QUOTAS.WORKFLOW_HISTORY_PRUNE_LIMIT) return -1 as FeatureReturnType[T];
		if (feature === LICENSE_QUOTAS.TEAM_PROJECT_LIMIT) return 999999 as FeatureReturnType[T];
